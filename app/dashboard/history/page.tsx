          ...
          <table className="min-w-full bg-white">
            ...
            <tbody>
              {HistoryList.map((historyItem) => {
                const { name, icon } = GetTemplateInfo(
                  historyItem.templateSlug
                );
                return (
                  <tr key={historyItem.id.toString()}>
                    ...
                    <td className="py-2 px-4 border-b border-gray-200 line-clamp-6">
                      {historyItem.aiResponse}
                    </td>
                    <td className="py-2 px-4 border-b border-gray-200">
                      {historyItem.createdAt}
                    </td>
                    <td className="py-2 px-4 border-b border-gray-200">
                      <CopyButton text={historyItem.aiResponse} />{" "}
                      {/* Use the CopyButton component */}
                    </td>
                  </tr>
                );
              })}
